...
/**
	This is a convenience variant of findPose() which also takes care of the
	detection.
...
*/
std::map<std::string, cv::Matx44d> findPose(const cv::Mat &pInputImage) const;
...
/**
	...
	\param pOmitOtherMarkers: if true, the pose of tags not specified in the
	configuration file will not be returned by findPose(). If false,
	setDefaultTagSize() can be used to set a coherent unit length among the
	markers.
	*/
void read3DConfiguration(
	const std::string &pFilename, bool pOmitOtherMarkers = false);

/**
	Sets the size of markers. By default, the result of findPose() uses the
...
/** 
	For accurate results, Chilitags3D can be provided the calibration data of
	the camera detecting the chilitags.
	\param pFilename the path to a file containing the calibration data, in the
	format output by OpenCV's 'calibration' sample.
